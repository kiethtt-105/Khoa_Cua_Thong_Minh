...
# ==================== EMAIL OTP (CHỈ CÒN MỘT CƠ CHẾ XÁC THỰC) ====================
...
class LoginOtpAttempt(models.Model):
    ...
    action = models.CharField(
        max_length=20,
        choices=[('EMAIL_OTP', 'Email OTP'), ('PASSWORD_RESET', 'Password reset')]
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
# ==================== MỚI THÊM CHO ĐỀ TÀI A2: MÃ PIN CỬA ====================
class DoorPinCode(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    device = models.ForeignKey(Device, on_delete=models.CASCADE, related_name='pin_codes')
    created_by = models.ForeignKey(User, on_delete=models.RESTRICT, related_name='created_pincodes')
